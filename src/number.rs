        ...
        match self {
            Number::Signed(val) => {
                ...
                if val >= lower_bound.try_into().unwrap() && upper_bound_check {
                    val.try_into()
                        .map_err(|e| DecodeError::Generic(format!("{:?}", e)))
                } else {
                    Err(DecodeError::Generic(format!(
...
pub fn read_number(buf: &mut &[u8]) -> Result<Number, DecodeError> {
    match rmp::decode::read_marker(buf)
        .map_err(|_| DecodeError::Generic("Unable to read marker for number".to_owned()))?
    {
        Marker::FixPos(val) => Ok(Number::Unsigned(val as u64)),
        Marker::FixNeg(val) => Ok(Number::Signed(val as i64)),
        Marker::U8 => Ok(Number::Unsigned(
            buf.read_data_u8().map_err(|_| DecodeError::Generic("Generic".to_owned()))? as u64,
        )),
        Marker::U16 => Ok(Number::Unsigned(
            buf.read_data_u16().map_err(|_| DecodeError::Generic("Generic".to_owned()))? as u64,
        )),
        Marker::U32 => Ok(Number::Unsigned(
            buf.read_data_u32().map_err(|_| DecodeError::Generic("Generic".to_owned()))? as u64,
        )),
        Marker::U64 => Ok(Number::Unsigned(
            buf.read_data_u64().map_err(|_| DecodeError::Generic("Generic".to_owned()))?,
        )),
        Marker::I8 => Ok(Number::Signed(
            buf.read_data_i8().map_err(|_| DecodeError::Generic("Generic".to_owned()))? as i64,
        )),
        Marker::I16 => Ok(Number::Signed(
            buf.read_data_i16().map_err(|_| DecodeError::Generic("Generic".to_owned()))? as i64,
        )),
        Marker::I32 => Ok(Number::Signed(
            buf.read_data_i32().map_err(|_| DecodeError::Generic("Generic".to_owned()))? as i64,
        )),
        Marker::I64 => Ok(Number::Signed(
            buf.read_data_i64().map_err(|_| DecodeError::Generic("Generic".to_owned()))?,
        )),
        Marker::F32 => Ok(Number::Float(
            buf.read_data_f32().map_err(|_| DecodeError::Generic("Generic".to_owned()))? as f64,
        )),
        Marker::F64 => Ok(Number::Float(
            buf.read_data_f64().map_err(|_| DecodeError::Generic("Generic".to_owned()))?,
        )),
        _ => Err(DecodeError::Generic("Invalid number type".to_owned())),
    }
}
